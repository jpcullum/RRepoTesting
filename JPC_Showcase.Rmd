...
stack_AQ <- stack(AirQuality[,1:4]) %>%
  ...
  mutate(col = ifelse(ind=="Ozone","dark blue",ifelse(ind=="Solar.R", "orange", ifelse(ind=="Wind","black","dark red")))) %>%
  mutate(fill = ifelse(ind=="Ozone","light blue",ifelse(ind=="Solar.R", "gold", ifelse(ind=="Wind","light gray","red"))))
...
ggplot(
  ...
  theme_hc()+
  scale_color_manual(values = c("light blue","gold","light gray","red"),
                     aesthetics = "fill"
  )
...
grid.arrange(
  ggplot(AirQuality, aes(DaySeries,Ozone_p))+
    geom_bar(stat="identity", col = "dark blue", fill = "light blue")+
    theme_hc()+
    ylab("Ozone")+
    xlab(""),
  ggplot(AirQuality, aes(DaySeries,Solar.R_p))+
    geom_bar(stat="identity", col = "orange", fill = "gold")+
    theme_hc()+
    ylab("Solar Radiation")+
    xlab(""),
  ggplot(AirQuality, aes(DaySeries,Wind_p))+
    geom_bar(stat="identity", col = "black", fill = "light gray")+
    theme_hc()+
    ylab("Wind Speed")+
    xlab("Experiment Day"),
  ggplot(AirQuality, aes(DaySeries,Temp_p))+
    geom_bar(stat="identity", col = "dark red", fill = "red")+
    theme_hc()+
    ylab("Temperature")+
    xlab("Experiment Day"),
nrow=2,
