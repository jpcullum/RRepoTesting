...
gg.Profits.month <- ggplot(DailySales) +
  ...
  theme(plot.title = element_text(hjust = 0.5, size = 12), 
        ...
        legend.background = element_rect(fill = "light gray")) +
  theme_hc()
...
### Conclusions
<br>
Overall, this document demonstrates the power of using R to easily take something as simple as a CSV of weekly sales and turn it into a powerful, simply weekly checkup tool. With more time and a specific goal, a complex analysis could be created. A check could even be incorporated to indicate whether a long or short report was desired. And, as mentioned earlier, this could easily be built as a R Shiny dashboard which would allow for a more interactive display and analysis. Once written, this code can be run as many times as needed, without any changes, and will provide a consistent report that can be cataloged and compared to past weeks. Raw number analysis can also be exported and automatically saved to folders named after the week or month the report was run for.
<br>
In the end, R provides a powerful tool to allow us to quickly, efficiently, and consistently review the large volumes of data every organization now encountered, and because it is relatively simple to work with it can be tailed to each individal task without issue.
<br>
<br>
